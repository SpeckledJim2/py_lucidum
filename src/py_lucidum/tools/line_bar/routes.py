...
from .query import chart, glm_overlay, table
...
def register(app: FastAPI, context: AppContext) -> None:
    ...
    async def glm_overlay_endpoint(request: Request) -> dict:
        context.check_token(request)
        payload = await request.json()
        try:
            started = time.perf_counter_ns()
            with context.dataset.lock:
                result = glm_overlay(
                    context.dataset,
                    payload,
                    feature_spec=getattr(app.state, "feature_spec", {}),
                )
            elapsed_ns = time.perf_counter_ns() - started
            result["timings"] = {
                "duckdb_ns": elapsed_ns,
    ...
    app.add_api_route("/api/line-bar/glm-overlay", glm_overlay_endpoint, methods=["POST"])
