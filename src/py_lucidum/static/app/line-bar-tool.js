...
export function createLineBarTool({
  ...
  function favouriteById(id) {
    return favourites.find((favourite) => favourite.id === id) || null;
  }
  ...
  function favouriteByStartupKey(key) {
    ...
    return favourites.find((favourite) => String(favourite.name || "").toLowerCase() === folded) || null;
  }
  ...
  async function refreshFavourites(options = {}) {
    try {
      ...
      renderFavouriteControl(error.message || "Favourites unavailable");
      return [];
    }
  }

  function renderFavouriteControl(errorMessage = "") {
    const select = el("lineBarFavouriteSelect");
    if (!select) return;
    select.innerHTML = "";
    const emptyLabel = errorMessage || (favourites.length ? "Choose" : "None");
    select.append(new Option(emptyLabel, ""));
    for (const favourite of favourites) {
      const suffix = favouriteValidationErrors(favourite).length ? " (invalid)" : "";
      select.append(new Option(`${favourite.name}${suffix}`, favourite.id));
    }
  ...
  async function applyFavourite(favourite, options = {}) {
    ...
    if (errors.length) {
      const message = `Line/Bar favourite "${favourite.name}" cannot be used. ${errors.join(" ")}`;
    ...
    try {
      ...
      const message = error.message || "Line/Bar favourite could not be restored.";
      setStatus(message, true);
      renderFavouriteControl();
      return message;
    }
  ...
  async function applySelectedFavourite(id) {
    const favourite = favouriteById(id);
    if (!favourite) return;
    await applyFavourite(favourite, { refresh: true });
  }

  async function applyStartupFavourite() {
    if (favouriteStartupApplied) return "";
    favouriteStartupApplied = true;
    const target = String(startupLineBarFavourite() || "").trim();
    if (!target) return "";
    if (!favouritesLoaded) await refreshFavourites();
    const favourite = favouriteByStartupKey(target);
    if (!favourite) {
      const message = `Line/Bar favourite not found: ${target}`;
      setStatus(message, true);
      return message;
    }
    return applyFavourite(favourite, { refresh: false });
  }

  function closeFavouritePopover() {
    const popover = el("lineBarFavouritePopover");
    if (!popover) return;
    popover.hidden = true;
    popover.innerHTML = "";
  ...
  function placeFavouritePopover() {
    const control = el("lineBarFavouritesControl");
    const popover = el("lineBarFavouritePopover");
  ...
  function selectedFavouriteManageIndex() {
    ...
    return favourites.findIndex((favourite) => favourite.id === selectedFavouriteManageId);
  }
  ...
  function updateFavouriteMoveControls() {
    ...
    if (downButton) downButton.disabled = selectedIndex < 0 || selectedIndex >= favourites.length - 1;
  }
  ...
  function updateFavouriteAddButton() {
    const input = el("lineBarFavouriteNameInput");
    const button = el("lineBarFavouritePopover")?.querySelector('[data-favourite-action="save-add"]');
    if (!input || !button) return;
  ...
  function renderFavouritePopover(mode = "manage", message = "", isError = false) {
    ...
    if (mode === "add") {
      popover.innerHTML = `
        <div class="line-bar-favourite-popover-head">
          <input id="lineBarFavouriteNameInput" class="line-bar-favourite-name-input" type="text" maxlength="120" placeholder="Name" />
    ...
    const rows = favourites.map((favourite) => `
  ...
  async function handleFavouritePopoverAction(action, button) {
    try {
      if (action === "save-add") {
        ...
        if (!name) return;
        const view = captureLineBarFavouriteView();
      ...
      if (action === "move-up" || action === "move-down") {
        ...
        if (index < 0 || nextIndex < 0 || nextIndex >= favourites.length) return;
  ...
  function bindFavouriteControls() {
    el("lineBarFavouriteSelect")?.addEventListener("change", (event) => {
      applySelectedFavourite(event.target.value);
    });
    el("lineBarFavouriteAddBtn")?.addEventListener("click", () => openFavouritePopover("add"));
    el("lineBarFavouriteMenuBtn")?.addEventListener("click", () => {
      const popover = el("lineBarFavouritePopover");
    ...
    el("lineBarFavouritePopover")?.addEventListener("click", (event) => {
    ...
    el("lineBarFavouritePopover")?.addEventListener("focusin", (event) => {
    ...
    el("lineBarFavouritePopover")?.addEventListener("input", (event) => {
      if (event.target.id === "lineBarFavouriteNameInput") {
        updateFavouriteAddButton();
        return;
      }
      const row = event.target.closest(".line-bar-favourite-row");
    ...
    el("lineBarFavouritePopover")?.addEventListener("keydown", (event) => {
      if (event.key === "Escape") closeFavouritePopover();
      if (event.key === "Enter" && favouritePopoverMode === "add") {
        const saveButton = el("lineBarFavouritePopover").querySelector('[data-favourite-action="save-add"]');
        saveButton?.click();
      }
    });
    document.addEventListener("click", (event) => {
      ...
      if (!control.contains(event.target)) closeFavouritePopover();
    });
    ...
    el("lineBarToolbar")?.addEventListener("scroll", () => {
      const popover = el("lineBarFavouritePopover");
      if (!popover || popover.hidden) return;
      placeFavouritePopover();
    });
