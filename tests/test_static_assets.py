...
"""
    ...
    def test_line_bar_favourites_static_contract(self) -> None:
        ...
        self.assertIn('id="lineBarFavouritesControl"', html)
        self.assertIn('id="lineBarFavouriteSelect"', html)
        self.assertIn('id="lineBarFavouriteAddBtn"', html)
        self.assertIn('id="lineBarFavouriteMenuBtn"', html)
        self.assertIn('id="lineBarFavouritePopover"', html)
        toolbar_start = html.index('id="lineBarToolbar"')
        first_control = html.index('class="control', toolbar_start)
        self.assertIn('id="lineBarFavouritesControl"', html[toolbar_start:first_control])
        self.assertIn(".line-bar-favourites-control", css)
        self.assertIn(".line-bar-favourite-popover", css)
        self.assertIn("grid-template-columns: minmax(120px, 1fr) 28px 28px;", css)
        ...
        self.assertIn("&uarr;", line_bar_js)
        self.assertIn("&darr;", line_bar_js)
        self.assertIn("&#10003;", line_bar_js)
        self.assertIn("&times;", line_bar_js)
        self.assertIn("data-original-name", line_bar_js)
        row_start = line_bar_js.index('<div class="line-bar-favourite-row')
        delete_index = line_bar_js.index('data-favourite-action="delete"', row_start)
        rename_index = line_bar_js.index('data-favourite-action="rename"', row_start)
        input_index = line_bar_js.index('class="line-bar-favourite-name-input"', row_start)
        self.assertLess(rename_index, input_index)
        self.assertLess(input_index, delete_index)
        self.assertNotIn('data-favourite-action="up"', line_bar_js)
        self.assertNotIn('data-favourite-action="down"', line_bar_js)
        self.assertNotIn(">Save</button>", line_bar_js)
        self.assertNotIn(">Up</button>", line_bar_js)
        self.assertNotIn(">Down</button>", line_bar_js)
        self.assertNotIn(">Delete</button>", line_bar_js)
        self.assertNotIn(">Add</button>", line_bar_js)
        self.assertNotIn(">Close</button>", line_bar_js)
        self.assertIn(".line-bar-favourite-action-button", css)
        self.assertIn(".line-bar-favourite-move-controls", css)
        self.assertIn("grid-template-columns: 28px;", css)
        self.assertIn("applyStartupFavourite", line_bar_js)
...
"""
    ...
    def test_index_uses_stable_local_asset_urls_and_disables_cache(self) -> None:
        ...
        self.assertIn('columnButton.className = "dataset-meta-column-link";', js)
        ...
        self.assertIn('id="lineBarExpandBtn" class="tab line-bar-icon-tab line-bar-expand-button"', html)
        self.assertIn('id="lineBarCopyBtn" class="tab line-bar-icon-tab"', html)
        self.assertLess(html.index('id="lineBarExpandBtn"'), html.index('id="lineBarCopyBtn"'))
        ...
        self.assertIn("lineBarFocusMode: false", js)
        self.assertNotIn("lineBarFocusPreviousSidebarVisible", js)
        self.assertIn("function setLineBarFocusMode(focused)", js)
        self.assertNotIn("setSidebarVisible", self.js_function_source(js, "setLineBarFocusMode"))
        self.assertNotIn("if (state.lineBarFocusMode && visible)", self.js_function_source(js, "setSidebarVisible"))
        self.assertIn("document.body.classList.toggle(\"line-bar-focus-mode\", focused);", js)
        self.assertIn("toggleLineBarFocusMode,", js)
        self.assertIn("body.line-bar-focus-mode #lineBarToolbar,", css)
        self.assertIn("button.classList.remove(\"active\");", js)
        self.assertIn("body.line-bar-focus-mode #lineBarCopyBtn {\n        order: 2;", css)
        self.assertIn("body.line-bar-focus-mode #lineBarExpandBtn {\n        order: 1;", css)
        self.assertIn("body.line-bar-focus-mode #chartTab {\n        order: 3;", css)
        self.assertIn("body.line-bar-focus-mode #tableTab {\n        order: 4;", css)
        self.assertIn("body.line-bar-focus-mode #tableWrap {\n        padding-top: 42px;", css)
        self.assertIn("body.line-bar-focus-mode #lineBarExpandBtn .line-bar-focus-icon-enter {\n        display: none;", css)
        self.assertIn("body.line-bar-focus-mode #lineBarExpandBtn .line-bar-focus-icon-exit {\n        display: block;", css)
    ...
    def test_glm_frontend_contains_real_tool_contract(self) -> None:
        ...
        self.assertIn('const GLM_TABULATION_SPLIT_STORAGE_KEY = "py_lucidum_glm_tabulation_sidebar_width_v2";', glm_js)
...
"""
    ...
    def test_gbm_frontend_contains_real_tool_contract(self) -> None:
        ...
        self.assertIn('const SHAP_SIDE_WIDTH_KEY = "py_lucidum_gbm_shap_side_width";', js)
        self.assertIn('const STACKED_SHAP_SIDE_WIDTH_KEY = "py_lucidum_gbm_stacked_shap_side_width";', js)
...
""")
    ...
    def test_line_bar_table_search_uses_complete_table_client_filter(self) -> None:
        ...
        self.assertIn("if (applyClientLineBarTableFilter({ requestKey })) return tableCacheData;", js)
    ...
    def test_sidebar_filter_accordion_header_and_row_meta_contract(self) -> None:
        ...
        self.assertIn('id="filterCollapseBtn" class="filter-header sidebar-section-header" type="button" aria-label="Expand FILTER" aria-expanded="false" title="Expand FILTER"', html)
        ...
        self.assertLess(html.index('<div class="filter-controls-row">', filter_start), html.index('id="filterSidebarClearBtn"', filter_start))
        ...
        self.assertIn("openSidebarSection: null", js)
    ...
    def test_sidebar_metric_section_sits_above_fixed_accordion(self) -> None:
        _, html_body = self.assert_no_store("/")
        css = self.app_css_contract()
        html = html_body.decode("utf-8")
        js = self.app_js_contract()
        ...
        self.assertLess(html.index('class="section sidebar-metric-section"'), html.index('data-sidebar-section="kpi"'))
        self.assertLess(html.index('data-sidebar-section="kpi"'), html.index('data-sidebar-section="gbm"'))
        self.assertLess(html.index('data-sidebar-section="gbm"'), html.index('data-sidebar-section="glm"'))
        self.assertLess(html.index('data-sidebar-section="glm"'), html.index('data-sidebar-section="filter"'))
    ...
    def test_filter_footer_and_sidebar_filter_controls_contract(self) -> None:
        ...
        self.assertLess(html.index('data-control="filterSelectionMode"'), html.index('id="filterSidebarClearBtn"'))
        self.assertLess(html.index('id="filterSidebarClearBtn"'), html.index('data-control="filterOperator"'))
...
""",
    ...
    def test_map_layer_control_uses_distinct_radio_groups(self) -> None:
        ...
        self.assertIn('<span class="slider-scale"><b>0</b><b id="mapLineWeightValue">1</b><b>5</b></span>', html)
        self.assertIn('<input id="mapLineWeight" type="range" min="0" max="5" step="1" value="1" />', html)
        self.assertIn('<label id="mapDotSizeControl" class="map-slider-control" hidden>', html)
        self.assertIn("<span>Dot size</span>", html)
        self.assertIn('<span class="slider-scale"><b>0</b><b id="mapDotSizeValue">1</b><b>5</b></span>', html)
        self.assertIn('<input id="mapDotSize" type="range" min="0" max="5" step="1" value="1" />', html)
        ...
        self.assertIn('<span class="slider-scale"><b id="mapHotspotsMinLabel">Bot</b><b id="mapHotspotsValue">All</b><b id="mapHotspotsMaxLabel">Top</b></span>', html)
        ...
        self.assertIn(".map-slider-grid.unit-mode #mapHotspotsControl {\n        grid-column: span 2;", css)
        ...
        self.assertIn('el("mapHotspotsMinLabel").textContent = unitMode ? "Bottom 10%" : "Bot";', js)
        self.assertIn('el("mapHotspotsMaxLabel").textContent = unitMode ? "Top 10%" : "Top";', js)
        ...
        self.assertIn("state.mapControlMoved = true;\n    state.mapControlPosition = panelPosition;", js)
        ...
        self.assertIn("mapControlCollapsed: false", js)
    ...
    def test_dataset_viewer_tool_static_assets_are_registered(self) -> None:
        ...
        self.assertIn("return { width: 96, minWidth: 72 };", dataset_js)
        self.assertIn("return { width: 128, minWidth: 104 };", dataset_js)
        self.assertIn("return { width: 150, minWidth: 96 };", dataset_js)
        ...
        self.assertIn("width: 220", dataset_js)
        self.assertIn("minWidth: 170", dataset_js)
        self.assertIn("width: 100", dataset_js)
        ...
        self.assertIn("function applyTransposedSearch(query)", dataset_js)
        self.assertIn('datasetTable.setFilter((row) => transposedColumnMatchesSearch(row, query));', dataset_js)
        ...
        self.assertIn("function transposedColumnMatchesSearch(column, query)", dataset_js)
        ...
        self.assertIn('el("datasetViewerFilter").textContent = label;', main_js)
...
"""
    ...
    def test_app_js_contains_unit_point_map_controls(self) -> None:
        ...
        self.assertIn("function unitPointRadiusScale(value = state.mapDotSize)", js)
        self.assertIn("const sliderValue = Number(state.mapDotSize);", js)
        self.assertIn("if (!Number.isFinite(sliderValue)) return MAP_UNIT_POINT_RADIUS_MULTIPLIER;", js)
        self.assertIn("const scale = sliderValue <= 1\n      ? 0.4 + (sliderValue * 0.6)\n      : 1 + ((sliderValue - 1) / 4);", js)
        self.assertIn("return scale * MAP_UNIT_POINT_RADIUS_MULTIPLIER;", js)
        self.assertIn("if (Number.isFinite(sliderValue) && sliderValue <= 0) return MAP_UNIT_POINT_MIN_RADIUS;", js)
        self.assertIn("return unitPointRadiusForZoom(zoom) * unitPointRadiusScale(sliderValue);", js)
