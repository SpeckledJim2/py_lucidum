...
"""
    ...
    def test_line_bar_favourites_static_contract(self) -> None:
        ...
        self.assertIn('const FAVOURITE_SCOPES = new Set(["metrics", "metrics_filter", "line_bar_view", "histogram_view", "map_view"]);', main_js)
        self.assertIn('FAVOURITE_SCOPES = {"metrics", "metrics_filter", "line_bar_view", "histogram_view", "map_view"}', favourites_py)
        ...
        self.assertIn('return tool === "line_bar" || tool === "histogram" || tool === "uk_map";', main_js)
...
for (const tool of ["column_profile", "dataset_viewer", "glm", "gbm", "specs"]) {
...
""")
    ...
    def test_sidebar_metric_section_sits_above_accordions_for_metric_tools(self) -> None:
        _, html_body = self.assert_no_store("/")
        css = self.app_css_contract()
        html = html_body.decode("utf-8")
        js = self.app_js_contract()
        ...
        self.assertIn('return ["line_bar", "histogram", "uk_map", "glm", "gbm"].includes(state.tool);', js)
