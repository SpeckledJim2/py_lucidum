...
"""
    ...
    def test_gbm_frontend_contains_real_tool_contract(self) -> None:
        ...
        self.assertIn("function toolRendersMetricSummaries(tool = state.tool)", js)
...
""")
    ...
    def test_app_js_contains_unit_point_map_controls(self) -> None:
        ...
        self.assertIn("if (sliderValue <= 1) return 0.5 + (sliderValue * 0.5);", js)
        self.assertIn("return 1 + ((sliderValue - 1) / 4);", js)
        self.assertIn("return unitPointRadiusForZoom(zoom) * unitPointRadiusScale(state.mapLineWeight);", js)
