...
""")
    ...
    def test_map_layer_control_uses_distinct_radio_groups(self) -> None:
        ...
        self.assertIn("function mapFloatingExpandedTopRightPosition()", js)
        self.assertIn("const panelPosition = {\n      left: Math.max(margin, frame.width - panel.offsetWidth - margin),\n      top: topMargin,", js)
        self.assertIn("state.mapControlMoved = true;\n    state.mapControlPosition = topRight.panel;", js)
        self.assertIn("setMapFloatingCollapsedPosition(topRight.button.left, topRight.button.top);", js)
