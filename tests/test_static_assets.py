...
"""
    ...
    def test_uk_map_lazy_loads_the_local_maplibre_module(self) -> None:
        ...
        self.assertIn('attributionControl: {}', adapter)
        self.assertIn('maplibreLogo: false', adapter)
        self.assertNotIn('maplibreLogo: true', adapter)
        self.assertIn('attribution.classList.remove("maplibregl-compact-show")', adapter)
        self.assertIn('rel.add("noopener")', adapter)
        ...
        self.assertIn('this.hoverFillLayerId = objectId("lucidum-geojson-hover-fill")', adapter)
        self.assertIn('this.hoverLineLayerId = objectId("lucidum-geojson-hover-line")', adapter)
        self.assertIn('["boolean", ["feature-state", "hovered"], false]', adapter)
        self.assertIn('hoverFillOpacity: Number.isFinite(hoverFillOpacity) ? hoverFillOpacity : 0.12', adapter)
        self.assertIn('setHoveredFeature(layer = null)', adapter)
        ...
        self.assertIn('Powered by <a href="https://www.esri.com/"', map_source)
        self.assertIn('Source: Esri, Vantor, Earthstar Geographics, and the GIS User Community', map_source)
        ...
        self.assertIn('hoverStyle: mapPolygonHoverStyle', map_source)
        self.assertIn('analysisHoverFillLayerId', map_source)
        self.assertIn('analysisHoverLayerId', map_source)
        ...
        self.assertIn('esri: "blank"', map_source)
        self.assertIn('osm: "blank"', map_source)
        self.assertNotIn('value="esri"', index)
        self.assertNotIn('value="osm"', index)
        ...
        self.assertIn('new L.maplibregl.FullscreenControl({', map_source)
        self.assertIn('container: document.querySelector("main")', map_source)
        self.assertIn('pseudo: usePseudoMapFullscreen()', map_source)
        self.assertIn('function usePseudoMapFullscreen()', map_source)
        self.assertIn('/iPad|iPhone|iPod/.test(userAgent)', map_source)
        self.assertIn('platform === "MacIntel" && window.navigator.maxTouchPoints > 1', map_source)
        self.assertIn('fullscreenContainer.querySelector("button").id = "mapFullscreen"', map_source)
        self.assertIn('mapFullscreenControl.on("fullscreenstart", handleMapFullscreenChange)', map_source)
        self.assertIn('mapFullscreenControl.on("fullscreenend", handleMapFullscreenChange)', map_source)
        self.assertIn('grid-template-rows: repeat(7, 34px);', map_styles)
        self.assertIn('.maplibregl-ctrl-attrib:not(.maplibregl-compact)', map_styles)
        self.assertNotIn('padding: 0 4px;', map_styles)
        self.assertIn('.map-native-navigation,', map_styles)
        self.assertIn('.map-native-fullscreen {', map_styles)
        self.assertIn('display: contents;', map_styles)
        self.assertIn('main.maplibregl-pseudo-fullscreen {', map_styles)
        self.assertIn('height: 100dvh !important;', map_styles)
        self.assertIn('env(safe-area-inset-bottom)', map_styles)
        self.assertIn('#mapFullscreen .maplibregl-ctrl-icon {', map_styles)
        self.assertIn('#mapFullscreen.maplibregl-ctrl-shrink .maplibregl-ctrl-icon {', map_styles)
        self.assertIn('background-color: currentColor;', map_styles)
        self.assertIn('.map-native-fullscreen button:focus {', map_styles)
        self.assertIn('color: var(--text);', map_styles)
        self.assertIn('mask-image: url("data:image/svg+xml,', map_styles)
...
"""
    ...
    def test_uk_map_control_strip_markup_starts_expanded(self) -> None:
        ...
        self.assertEqual(index.count('name="baseMap"'), 4)
