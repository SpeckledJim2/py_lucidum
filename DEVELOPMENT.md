...
- Third-party browser libraries are vendored under `src/py_lucidum/static/vendor/`. Core ECharts and Leaflet are loaded locally from `index.html` because the default app tools need them at startup. Histogram lazy-loads Tabulator for the metrics grid. GLM lazy-loads Ace for formula editing. GBM lazy-loads Tabulator for editable grids and D3 for tree diagrams. ECharts GL is lazy-loaded through `static/app/shared/echarts-gl.js` for GBM SHAP and two-feature Line/Bar surface plots.
...
- Favourites persist `denominatorSource` beside the existing denominator string. Legacy favourites resolve against `dataset`; a saved GLM or GBM denominator is a logical active-model reference and resolves to the currently active source of that type. Model activation invalidates metric-consuming caches, rebinds only a matching model kind, refreshes the visible tool once, and leaves inactive tools stale until opened. Deleting the last matching model preserves an unavailable selection with an actionable warning.
...
- Two-feature chart rows expose `group0`/`group1`, sort values, missing flags, and numeric range metadata plus `volume`, `row_count`, and every `respN` value. Missing factor groups remain chartable; missing continuous values remain in table data but are omitted by surface/line renderers with a warning. The 10,000 grouped-cell chart cap remains in force, line plots show at most 80 factor series, and surfaces reject dense grids above 40,000 cells; all guardrail messages direct users to the uncapped server-backed table.
...
- Line/Bar favourite version 2 stores `tailPercent` inside each grouping, keeps top-level `tailPercent` as the Feature 1 compatibility alias, and also stores `plotMetric` plus optional `heatmapLabels` (`none`, `actual`, `weight`, or `both`). Legacy two-feature views copy their shared tail percentage to both groupings; legacy views default heatmap labels to `none`, and legacy `x`/`xSource` plus band/quantile/date fields normalise to one grouping. A stale optional Feature 2 is a non-blocking warning and is dropped during frontend fallback; a stale Feature 1 remains a blocking validation error.
...
- Chart requests are capped at 10,000 x-axis groups or two-feature grouped cells after the relevant Line/Bar grouping pipeline. Chart responses include `group_count`, `max_groups`, and `groups_truncated`; truncated chart responses warn that the table search still covers all groups.
...
- Histogram uses the shared launch-collapsed 50px settings strip and full-bleed workspace. On desktop, the chart is on the left and its metrics table starts at the 240px minimum width so the divider is initially as far right as possible; the pointer/touch/keyboard-resizable width is session-only and is not part of Histogram favourites. At viewport widths up to 900px, the divider hides and the metrics table stacks above the chart.
...
- The response includes bin rows, selected/requested/effective binning metadata, metric rows, `stats_exact` / `stats_sampled_count` metadata, exact row-count metadata, denominator metadata, warnings, and timings. The frontend uses width-aware integer x-axis labels, 1/2/5-power-of-ten intervals on linear y-axes, outlines charts with at most 200 bins, optionally renders fitted 7-10px labels matching the plotted sum/probability value, and renders offset mean/median reference lines labelled with their KPI-formatted values beside a compact Tabulator metrics grid.
