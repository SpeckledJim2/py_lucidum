...
- Table view uses `POST /api/line-bar/table` and is always server-backed. The table endpoint accepts the normal Line/Bar request fields plus `tableSearch`, `tablePage`, and `tablePageSize`; the default page size is 1,000 groups, and there is no table-wide result cap.
