...
- GLM artifacts are stored beside the source dataset under `.lucidum/models/glm/`, with `manifest.json`, `estimator.pkl`, `formula.txt`, `coefficients.parquet`, `predictions.parquet`, and `diagnostics.json`. Tabulation builds add `tabulation_manifest.json`, `tabulations/*.parquet`, and `tabulated_predictions.parquet`.
