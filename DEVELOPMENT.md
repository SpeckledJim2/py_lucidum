...
- App map assets load locally, including MapLibre GL and bundled UK GeoJSON. MapLibre requires WebGL2. Nonblank base-map backgrounds are a separate external tile dependency: OSM, Esri/Aerial, Light, and Dark layers fetch tiles from their configured third-party providers, while `Blank` makes no external tile requests.
