...
- Area and sector geometry use MapLibre GeoJSON sources and WebGL fill/line layers with hover tooltips and click popups. Pointer hover sets transient per-feature state, draws a low-opacity accent fill below vector-basemap linework, and draws a stronger accent outline above the normal analytical border without changing the underlying KPI fill; leaving the polygon clears that state. Informational tooltips and all of their descendants are pointer-transparent so a moving tooltip cannot generate a delegated layer-leave event and transiently clear the polygon state. Units retain their canvas-specific hit testing and do not use the polygon hover layers.
