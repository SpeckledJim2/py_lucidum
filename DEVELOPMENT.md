...
- The floating map panel uses the shared exact KPI match for the current dataset Numerator and normalized Denominator. It shows the KPI name with wrapping when matched and removes the heading from layout when unmatched; model-backed Denominators do not match dataset KPI rows.
...
- App map assets load locally, including MapLibre GL and bundled UK GeoJSON. MapLibre requires WebGL2. Nonblank base-map backgrounds are a separate external dependency: OSM, Esri/Aerial, and CARTO Light/Dark fetch raster tiles, while OpenFreeMap Positron/Dark load hosted vector styles and `Blank` makes no external tile requests. Replacing a vector style recreates every registered Lucidum source and layer against the settled style without duplicating interaction bindings, and a generation guard prevents an older style load from winning a rapid switch. OpenFreeMap symbol layers with a `text-field` return to the foreground in their original order after analytical layers are restored; CARTO retains separate no-label and label-only raster layers. Both light/dark pairs follow the app theme.
...
- Unit points draw into a hidden HTML canvas that MapLibre consumes as a non-animating, geographically anchored canvas source, with a hit grid for hover tooltips and click popups. MapLibre transforms the completed canvas texture continuously during panning, then the layer redraws and reanchors after movement ends; this avoids redrawing a large unit set on every pointer event. Zooming hides the old texture, redraws at the settled zoom, uploads one hidden frame, and reveals only the current zoom generation so rapid zooms cannot flash stale dots. The settled redraw derives a smoothly interpolated radius from the MapLibre zoom at every Dot size setting; the persisted 1-10 control is a relative multiplier rather than a fixed pixel radius. CARTO Light/Dark use separate no-label base and label-only raster layers so labels remain above polygon and unit data. Compact unit responses retain every valid-coordinate unit in stable postcode order, including a null KPI value when that metric cannot be plotted. Unit requests use a 25%-padded rectangular viewport when the current view does not already cover the complete unit layer; DuckDB applies its validated latitude/longitude bounds to raw rows before row counts, metric summaries, and postcode aggregation. The bounds are part of the geometry identity, so KPI-only changes reuse aligned metrics while the viewport remains inside the same coverage. Zooming in remains client-only and adopts a materially smaller coverage on the next data-changing request; zooming out or panning beyond the coverage retains the completed layer state but hides its now-incomplete canvas source while a debounced request replaces it with a larger padded extent. `Fit UK` deliberately obtains the unbounded result. Once the browser has geometry for the active source/filter/coordinate/bounds identity, KPI-only requests return aligned metric arrays without resending postcode keys, row counts, or coordinates. The existing layer then reuses typed Web Mercator coordinates and a 256-by-256 geographic index so redraws project and paint only cells intersecting the padded viewport. These bounds, compact arrays, geometry reuse, quantile sampling, and spatial indexes remain necessary with MapLibre because they reduce query, transfer, CPU drawing, memory, and hit-testing work rather than only compensating for Leaflet. Whole-UK queries and redraws still visit every applicable row/visible point. Unit colour thresholds and the Low/High extremes ranking are scoped to the loaded coverage, which is regional while zoomed in and national after `Fit UK`. Colour thresholds use a deterministic sample of at most 100,000 aligned values while retaining the exact finite minimum and maximum; smaller unit sets and area/sector layers remain exact.
