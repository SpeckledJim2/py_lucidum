...
- Python:
  ...
  - `py_lucidum.smooth_postcode_sectors(...)`
...
- External compatibility examples:
  ...
  - `examples/04_external_double_lift_demo.py [config-path]`
  - `examples/postcode_sector_smoothing_demo.py` is a standalone public-API
    example that smooths average demo `PREMIUM` after filtering to
    `SAMPLE = 'training'`, writes under `local/`, and is deliberately outside the
    numbered synced workflow set. Run it with
  ...
  - The optional 04 Double Lift script is a cross-model workflow rather than a
    fourth step in either build sequence. Its comparison YAML names two exact
    build-YAML paths; each path resolves its own result root, family, and model
    ID. It never follows an active-model marker. Reports retain the configured
    Baseline/Challenger order as `Challenger / Baseline`, align predictions by
    `__lucidum_row_id`, and prominently show the selected SAMPLE population.
...
- Feature spec CSV files must start with `Feature,Grouping`. Reserved metadata columns immediately after `Grouping` are `Monotonicity`, `Base`, `min`, `max`, `banding`, `chart_banding`, `chart_quantiles`, `chart_low_weights`, `chart_missings`, `chart_labels`, `chart_sort`, `chart_transform`, `chart_sigma`, `chart_date_bucket`, and `chart_empty_periods`; scenario columns start after the contiguous reserved metadata block. Optional `Monotonicity` accepts blank, case-insensitive `Increasing`/`1`, or `Decreasing`/`-1`, and nonblank values require a known numeric dataset feature. Older specs without these metadata columns remain valid and treat every column after `Grouping` as an ordered GBM scenario name. Scenario cells include the feature when they contain the word `feature`, case-insensitive.
...
- When compatible saved primary GLM/GBM prediction sources exist, Line/Bar pins a gold `Prediction ratio…` action above the x-axis features. Its anchored Baseline and Challenger choosers include only models whose source-aware training Numerator/Denominator exactly match the selected pair, grouped under GLM/GBM, followed by every readable numeric dataset column under OTHER. The chooser states the active pair and `Challenger / Baseline` calculation. Swap reverses any two distinct selections. Apply selects the dynamic `prediction_ratio` x-axis plus ordered Baseline/Challenger Expected responses while clearing Feature 2. Each dynamic source side is either `<model-family>:<model-id>` or `other:<utf8-column-name-as-lowercase-hex>`, so the complete id is `model_ratio:prediction_ratio:<challenger-side>:<baseline-side>` and arbitrary column names cannot alter source syntax. Prediction sidecars and dataset rows align by `__lucidum_row_id`; a null or zero Baseline produces a null ratio. Positional bindings retain a selected model's training scope and read OTHER columns from the same scoped dataset rows. The provider is registered when either modelling family is enabled and publishes no eager dynamic schema rows; the selected frontend column and existing request `xSource` carry the identity. The legacy active-pair `model_ratio:gbm_to_glm_ratio:<gbm-id>:<glm-id>` source remains supported for URLs and favourites.
- Mixed-source relations project every distinct `(source id, physical column)` through a collision-free internal column name before metric SQL is built. Same-family models therefore remain numerically independent even though their sidecars both expose `glm_prediction` or `gbm_prediction`; chart/table payloads, labels, warnings, filters, and favourites continue to expose only the original public column and source identities. Positional and `__lucidum_row_id` joins use the same projection contract.
...
- Generated Line/Bar fields are semantic model references, not generic same-name fallbacks. Ordinary x-axis `glm_prediction` and related GLM fields bind to the current active GLM source, GBM equivalents bind to the current active GBM source, and legacy `gbm_to_glm_ratio` binds to its active pair. Inactive Expected model rows bind to their exact saved sources, active-family Expected rows retain the existing activation-following behavior, and both are admitted only when their training metrics match the selected KPI. An explicit `modelComparison` binds the dynamic ratio and both primary Expected responses to exact saved model ids; later model activation does not replace a compatible pair, while KPI changes remove incompatible Expected sources, clear the pair, and use the normal x-axis fallback. Reconciliation must never replace an incompatible or missing exact Expected source with a different model merely because both artifacts expose the same physical prediction-column name. GLM/GBM activation, deletion, and replacement invalidate Line/Bar before schema reload; reverse schema completions cannot commit, and rapid activations are serialized and coalesced to the latest requested model. When a required exact source disappears, favourite validation identifies the missing Baseline or Challenger rather than silently substituting another model.
...
- UK Mapping uses the shared exact 50px control-strip height above the full-bleed map. Four headerless image tiles provide Blank, Aerial, OpenFreeMap Light, and OpenFreeMap Dark; Resolution and Palette use the same full-height image-over-label treatment. Resolution-specific Labels, Smooth, or Dot Size controls precede the area/sector Border chooser (`Off=0`, `Thin=1`, `Bold=3`), the Strength chooser (`Faint=0.2`, `Medium=0.6`, `Solid=1.0`), Extremes, and Postcode search. Legacy favourite line weights and opacity values are mapped to the nearest supported presentation. The controls region scrolls horizontally with the shared edge fades. A separate 16px full-width information strip below it presents the exact KPI match, geometry/plotted and missing/unmatched state, row count, and filter clear command on one line using 11px text. The KPI is omitted when there is no exact match. Complete truncated text remains in titles and the live status. The viewport-control chevron immediately above the native MapLibre NavigationControl toggles both strips, reverses direction and accessible text, retains session state across tool switches, and schedules only a local camera-preserving resize. The native compass immediately below Zoom Out retains MapLibre's bearing-synchronised icon and north-reset action; Lucidum supplies pitch-free angular pointer/touch tracking so equal cursor arcs rotate equally in every quadrant. The native MapLibre fullscreen control sits below the UK and London shortcuts and targets the complete `main` mapping workspace, retaining the map strips, legend, and attribution while excluding the app header, sidebar, and footer. Desktop browsers use native fullscreen when available; iPad, iPhone, and iPod touch devices force MapLibre's pseudo-fullscreen mode to avoid Apple's browser-owned downward-swipe dismissal gesture. Existing document overflow and overscroll containment prevent the fixed fullscreen workspace from scrolling, while fullscreen changes preserve the camera and resynchronise the map viewport. Collapse and fullscreen state are not part of favourites. Model-backed Denominators do not match dataset KPI rows.
- Map view favourites store camera `bearing` with centre and zoom. Missing or invalid bearings in legacy favourites normalize to `0` (north), including legacy views without a saved centre/zoom.
- UK Mapping detects binary metrics from all drawable post-aggregation values, with classification cached by response and geometry identity. Numeric 0/1 and booleans (including single-class results) use Low / All / High whole-class selection, fixed palette endpoints, and a two-class KPI-formatted legend. Missing/non-finite values are excluded and must never be coerced to zero. Units use the complete aligned point array, not the quantile sample or viewport; polygons use geometry-matched rows. Non-binary metrics retain percentage ranking. Empty metrics disable Extremes without changing its stored value. The existing favourite `hotspots` field remains numeric: its sign selects the binary class, and button clicks store -9/0/9. Class selection only redraws locally, retaining muted context, hover, and camera state.
...
- Sector smoothing uses a borderless six-option `-`, `N1`–`N5` chooser and a committed shared-edge adjacency sidecar generated from the bundled sector GeoJSON, then pools already-aggregated numerator and denominator values across sectors reachable within the selected neighbour depth. The interactive summary requests only the displayed depth. When smoothing is active, all sidecar sectors are smoothing targets, so sectors with no original data can be filled from valid neighbours. Raw sector fields remain available in the API rows for popup context.
- `py_lucidum.smooth_postcode_sectors(...)`, the interactive map summary, and `POST /api/uk-map/sector-smoothing` share the same DuckDB smoothing relation. The public writer accepts one physical Parquet source, physical sector/numeric metric columns, an optional valid DuckDB filter, and an explicit distinct output path. Omitting its denominator means valid Numerator-row count. It atomically writes `postcode_sector`, raw numerator/denominator sums, `unsmoothed`, `smooth_n1`–`smooth_n5`, and the pooled `numerator_n1`–`numerator_n5` and `denominator_n1`–`denominator_n5` sums in postcode order. Blank sectors are ignored; filtered nonblank sectors must be uppercase canonical values containing one space. The output contains every bundled geometry target plus valid source-only targets, whose smoothed values and component sums use the raw fallback.
- The sector-only **-> .parquet** action sits in the `Smooth` heading without changing the fixed control-strip height or moving the N1–N5 row. It calculates all five levels without refreshing the map and reports success/failure through the transient shared toast, showing a compact artifact filename while retaining the full absolute path as accessible detail. App exports support every map data source and model-backed Numerator/Denominator accepted by the normal map request. Deterministic artifacts live under `.lucidum/datasets/<slug>/<signature>/uk_map/sector_smoothing/`; identical normalized source/metric/filter specifications atomically replace the same file, while different specifications use separate 12-character hashes. These artifacts are not registered as Lucidum sources.
...
- Area and sector geometry use MapLibre GeoJSON sources and WebGL fill/line layers with hover tooltips and click popups. The immutable frontend postcode-area-name lookup exactly covers the 124 bundled areas without adding repeated names to GeoJSON or map-summary payloads. Area, sector, and unit hover cards show the complete active postcode, its postcode-area name, the formatted KPI, and the KPI denominator: `N` for the valid contributing-row count or the configured Weight label and total. Smoothed sectors show the pooled denominator used by the displayed value. Detailed click popups remain unchanged. Pointer hover sets transient per-feature state, draws a low-opacity accent fill below vector-basemap linework, and draws a stronger accent outline above the normal analytical border without changing the underlying KPI fill; leaving the polygon clears that state. Informational tooltips and all of their descendants are pointer-transparent so a moving tooltip cannot generate a delegated layer-leave event and transiently clear the polygon state. Polygon and unit hover paths reuse unchanged tooltip HTML while the pointer remains on the same content, updating only its position so pointer motion does not repeatedly rebuild the card. Units retain their canvas-specific hit testing and do not use the polygon hover layers.
- Unit points draw into a hidden HTML canvas that MapLibre consumes as a non-animating, geographically anchored canvas source, with a six-CSS-pixel minimum hit radius for hover tooltips and click popups. MapLibre transforms the completed canvas texture continuously during panning, zooming, and bearing changes, then the layer redraws and reanchors after movement ends; this avoids redrawing a large unit set on every animation frame. Settled drawing and hit testing rotate cached Web Mercator offsets around the viewport centre with the current bearing, and spatial culling covers all four rotated viewport corners. During zoom, the previous texture remains visible for continuity while hit testing is disabled; it may briefly scale or soften before the correctly sized settled texture replaces it. Generation guards prevent an older rapid-zoom redraw from winning. Units mode uses `dotSizeMode` (default `adaptive` or `min`) and `dotSizeLevel` (integer 1–10, default 5). The `Dot Size (N)` heading has smaller/larger chevrons. Level 1 shares the Min rendering path and paints one snapped physical pixel regardless of zoom or point count. Levels 2–10 adjust Adaptive diameter by N/5 after resolving its existing physical-pixel baseline, with a one-physical-pixel floor; level 5 preserves the original rendering exactly. The chevrons disable at their respective bounds; Min shows only `Dot Size`, hiding the number and chevrons while retaining N for return to Adaptive. N survives tool/resolution changes and favourites; a changed N clears the active favourite selection and redraws locally without moving the camera or fetching geometry. Legacy favourites containing `dotSize` still load as Adaptive; missing or invalid `dotSizeLevel` defaults to 5, and finite numeric levels are rounded and clamped to 1–10. New favourites write `dotSizeMode` and `dotSizeLevel`, including the remembered level in Min, without writing legacy `dotSize`. Min paints every eligible visible unit into one snapped 1-by-1 backing-store pixel regardless of device scale. Adaptive uses the complete filtered `point_summary.plotted_count` (falling back to the aligned finite-value count) and a non-mutating fitted-extent zoom computed with the same padding and maximum zoom as `Fit UK`. At the fitted zoom, dense point sets use one physical pixel. At close zoom, the cap is 10 CSS pixels for 500,000 points and logarithmically tapers to 8 CSS pixels by 1.5 million points; 100 or fewer remain 6 CSS pixels, with intermediate counts logarithmically interpolating the original dense and sparse anchors. Growth from baseline to cap uses smoothstep over six zoom levels. Zoom, rotation, resize, effective device-pixel-ratio changes, mode changes, and favourite restoration redraw locally without fetching geometry. Compact unit responses retain every valid-coordinate unit in stable postcode order, including a null KPI value when that metric cannot be plotted. Units mode always requests the complete point set after the active global filter, with no camera bounds, and retains that geometry while the user pans, zooms, rotates, or uses `Fit UK`; camera-only movement therefore performs no map-summary API request. Once the browser has geometry for the active source/filter/coordinate identity, KPI-only requests return aligned metric arrays without resending postcode keys, row counts, or coordinates. The existing layer reuses typed Web Mercator coordinates and a 256-by-256 geographic index so settled redraws project and paint only cells intersecting the padded visible canvas. Compact arrays, geometry reuse, quantile sampling, and spatial indexing reduce transfer, CPU drawing, memory, and hit-testing costs; initial whole-layer queries and index construction still visit every applicable row and valid-coordinate point. Unit colour thresholds and the Low/High extremes ranking are scoped to the complete filtered point set rather than the current camera view. Colour thresholds use a deterministic sample of at most 100,000 aligned values while retaining the exact finite minimum and maximum; smaller unit sets and area/sector layers remain exact.
...
- The wheel force-includes exactly the seven numbered external workflows and four
...
- The external GBM contract fits LightGBM directly and writes the current
  ...
  training. Feature Specification monotonicity is enabled by the optional
  default-true `features.use_monotonicity` switch and is derived in fitted-feature
  order; raw constraint-vector parameters are rejected. `parameters.json` must
  include every Model navigator parameter and
...
- `double_lift_chart` accepts two exact saved GLM/GBM folders, registers each
  source independently, and returns the standard Line/Bar payload grouped by
  Challenger/Baseline prediction ratio. Same-family predictions must retain
  source-scoped query aliases, while public model labels and source IDs remain
  unchanged. A zero or missing Baseline yields a missing ratio.
...
- The 03 GBM summary counts only rows joined to a saved prediction with a
  ...
  SHAP-to-Gain rule as Line/Bar and includes fitted monotonicity from the saved
  feature configuration. GBM SHAP-only report titles append that same fitted
  monotonicity when nonblank; Actual-versus-Expected titles remain unchanged.
  The performance table maps manifest `gini_tr`, `gini_te`, and `gini_vl` to
  the configured Training, Test, and Validation roles and formats them to four
  decimal places. Its visible Parameters table summarizes the fitted
  `monotone_constraints` vector by total, Increasing, and Decreasing counts;
  preserve the exact vector in `parameters.json` and the embedded report payload.
...
- GLM training derives required predictor columns from Formulaic, adds only the selected response, denominator, physical `SAMPLE` column when present, and offset-expression columns, and projects those columns plus `__lucidum_row_id` through DuckDB into Polars. `SAMPLE` is required even for an **All** fit so split Gini remains independent of fit scope. Do not restore the old all-readable-column pandas load or wide frame copies; formula `.` remains the explicit case where every available source column is required.
- New GLM and GBM builds calculate `gini_tr`, `gini_te`, and `gini_vl` from final predictions through `py_lucidum.model_metrics`. The exact rate/exposure, tie, normalization, SAMPLE, and undefined-value contract is documented once in the User Guide's [Normalized Gini section](docs/user-guide.md#normalized-gini). GLM persists the values in `diagnostics.json`; GBM persists them in `manifest.json`; list/config/mutation readers must expose all three as nullable top-level model fields so legacy artifacts render `--`. Undefined present partitions append non-blocking artifact warnings. Gini must not enter LightGBM evaluation, early stopping, or grid selection.
- `examples/external_model_results.py` deliberately remains independent of `py_lucidum` imports but mirrors the canonical [Normalized Gini contract](docs/user-guide.md#normalized-gini) for official external GLM and GBM artifacts, mapping configured sample labels to the three canonical roles.
...
- Ordinary GLM Tabulations model switches reuse the loaded `all_models` status payload, derive the selected table union locally, retain the existing model grid, and issue only the table/plot request. Full configuration refreshes remain required after model or tabulation artifact mutations. GLM and eligible 1D/2D GBM build, rename, and delete notifications mark that shared inventory stale even while Tabulations is hidden, while activation updates its cached active marker; rename follows the selected model, deletion preserves surviving mixed selections, and an empty selection falls back to the active model then the first available model. Configuration refreshes are latest-request-wins and reconcile requested references against the response's authoritative `all_models` before deriving tables locally. Untabulated GBM eligibility scans are cached by tree-table path, size, nanosecond modification time, and best iteration so unchanged tree artifacts are not rescanned on those refreshes.
...
- The GLM Model navigator shows `Terms`, `Features`, `Interactions`, `Tabulated`, fitted `Rows`, and `Scope`; scope maps persisted `all`, `training`, and `training_test` values to `All`, `Training`, and `Training + Test`, with missing legacy scope treated as `All`. Missing legacy counts render blank, while tabulation renders `Yes` or `-`. Both the GLM and GBM navigators label the renameable model column `Name`, including their fallback tables.
...
- GBM training canonicalizes selected features in case-insensitive alphabetical order, with the original name as the deterministic tie-breaker, before projection, constraint-index generation, fitting, or artifact writing. The external GBM builder must apply the identical rule so UI table sorting and Feature Specification row order cannot change LightGBM feature indexes. Existing model artifacts retain their historical `features.json` order. Training then projects only the selected response, denominator, SAMPLE, init-score, and feature columns through DuckDB into Polars. The projection must restore ascending `__lucidum_row_id` after joining a generated SAMPLE sidecar; DuckDB join output order is not stable, and changing row order changes LightGBM's bounded bin-construction sample even when SAMPLE membership is identical. Stable sorted categorical mappings are encoded once into numeric Arrow columns shared by training, test, validation, scoring, and SHAP; logical/boolean columns use the same two-level categorical path, including Polars' lowercase `false`/`true` labels. GBM tabulation must compare source boolean values with those saved labels using the same normalization. LightGBM receives Arrow tables rather than pandas matrices. Keep pandas limited to compact tree/evaluation/config artifacts, tabulations, and exports, and do not restore large-row pandas frame copies.
...
- GBM exposes `monotone_constraints_method` in the Parameters grid with default `advanced` and validates `basic`, `intermediate`, or `advanced`, including grid-search combinations. A new configuration is seeded from optional Feature Specification `Monotonicity`; Feature-grid edits survive scenario application, and active saved-model values take precedence. Training generates the numeric vector only after canonical fitted-feature ordering. Raw monotone vectors and aliases are rejected and excluded from restored Parameter rows. Persist the configured method in model provenance, but create a separate LightGBM fit-parameter copy that omits both method and vector unless the derived vector contains a nonzero constraint; the external builder must apply the same rule.
...
- GBM `features.json` is the persisted source of truth for the trained model's input feature names in exact LightGBM training order. New fits write the canonical alphabetical order; readers must continue to respect historical artifacts in their saved order. GBM `feature_config.parquet` is an optional output/display artifact for the trained features, enriched with kind, monotonicity settings, Gain values, and optional `mean_abs_shap` values.
...
- UK map area, sector, and unit aggregation, alias defaults, coordinate validation, custom column defaults, selected-level smoothing, five-level Parquet exports, public-writer validation and atomic replacement, and app/model metric parity.
