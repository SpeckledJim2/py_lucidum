...
- Frontend tool ownership is split by file: `column-profile-tool.js`, `line-bar-tool.js`, `uk-map-tool.js`, `glm-tool.js`, `gbm-tool.js`, `gbm-shap-tool.js`, `gbm-shap-chart.js`, `gbm-stacked-shap-tool.js`, `gbm-stacked-shap-chart.js`, `gbm-tree-viewer.js`, and `model-tool-shell.js`.
...
- GLM frontend: read `static/app/glm-tool.js` and `static/styles/glm.css`; backend behavior is under `tools/glm/`.
- GBM frontend: read `static/app/gbm-tool.js` and `static/styles/gbm.css`; SHAP, Stacked SHAP, and tree UI have their own `gbm-shap-*`, `gbm-stacked-shap-*`, and `gbm-tree-viewer.js` files. Backend behavior is under `tools/gbm/`.
