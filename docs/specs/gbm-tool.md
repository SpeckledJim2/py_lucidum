...
- saved `parameters.json` is a LightGBM Python params dict, including objective, metric, and generated numeric monotone/interaction constraints when applicable; generated vectors are retained as exact training provenance but hidden from the editable Parameters grid, while semantic Feature monotonicity and interaction constraints are restored from fitted feature metadata and `manifest.json`; Lucidum-only init-score and training-mode metadata is also stored in `manifest.json`
...
- at the top of the interaction-constraint dropdown, an opt-in `Create constraint group LightGBM model .txt file(s)` checkbox requests one SHAP-centred LightGBM text sidecar for every selected group. It defaults off and is enabled only with at least one selected group and non-zero SHAP rows. Selecting `SHAP rows: 0` turns this optional export off and disables it until non-zero SHAP rows are selected again, without clearing the interaction constraints. Trained groups show either the verified maximum absolute raw-score error or `No trees`; edit-time group changes hide results that no longer describe the draft selection.
- right-clicking a Feature cell offers `Constrain to main effect only (1D)`, `Remove main-effect-only constraint`, and `Add pair interaction (2D)…`. Main-effect-only and pair markers are lock subscripts `1` and `2`. The pair action opens the separate `Interaction pairs` manager with that feature preselected; adding a pair selects both features. The manager has `Add pair interaction`, a live strict-policy summary, and `Allowed pair interactions (n)` sections; saved pairs render as `FeatureX × FeatureY` with an explicit `Remove` button, while an empty list says `No pair interactions added`. For a paired feature, replace the context-menu add action with one `Remove FeatureX × FeatureY pairwise interaction` action per pair. Main-effect-only/pair and pair/group overlaps are disabled in the UI and rejected by backend validation. Pair mode sends `feature_interaction_pairs`, persists as `mode: "pairs"` with `uncovered_policy: "singletons"`, and converts explicit pairs, main-effect-only features, and disjoint groups to LightGBM constraints plus one singleton for every uncovered selected feature. Those derived features show an automatic subscript-`1` lock but remain eligible for adding to a pair; their context menu includes display-only `Main effect only (automatic)`, and they are not sent as explicit main-effect constraints. Pair mode requires `num_leaves <= 3`; reject invalid scalar requests and skip invalid grid combinations. Group-only constraints retain their shared remainder. For legacy pair manifests without `uncovered_policy`, infer `singletons`, `remainder`, or `unknown` from saved feature order and numeric constraints; display a historical remainder state until the user edits feature constraints, then show the strict retraining draft.
...
- expose `monotone_constraints_method` with `advanced` as the default and `basic`, `intermediate`, and `advanced` as the only scalar or grid-search choices. Generate `monotone_constraints` exclusively from selected Feature rows after canonical feature ordering; reject raw vectors/aliases and never restore the vector as an editable Parameter row. Persist the configured method for every build, but pass the method and vector to LightGBM only when the derived vector contains a nonzero constraint.
- the Parameters header has a copy command that writes the current scalar values as a pretty JSON object suitable for `lgb.train(params=...)`. Exclude `init_score`, which belongs on `lgb.Dataset(init_score=...)`, and generated `interaction_constraints`, which are regenerated from the Feature controls immediately before training. Block copying while any parameter contains Lucidum grid-search braces.
...
When constraint-group model creation is requested, save the main `model.txt` first, then save each retained group beside it as `model_<group>.txt`. Replace unsafe or non-ASCII filename characters with `_`, cap the readable group stem at 80 characters, and append an eight-character SHA-256 suffix after truncation or a case-insensitive collision. Reload each output with LightGBM and predict with `raw_score=True` on the exact encoded rows used for saved SHAP values. Persist the maximum absolute difference from the corresponding `<Grouping>_INTERACTION_GROUP` column without a pass/fail tolerance. Loading, shape, finiteness, extraction, or prediction failures fail the training job; a group with no fitted trees omits its file and records `no_trees` without discarding the main model.
...
- when feature interaction constraint groups are selected, grouped SHAP contribution columns in `shap_values.parquet` named `<Grouping>_INTERACTION_GROUP`; main-effect-only and automatically generated uncovered-feature constraints do not create grouped SHAP columns
- optional constraint-group LightGBM sidecars beside `model.txt`, with requested/enabled state and per-group status, relative artifact name, retained tree count, verified row count, and full-precision maximum absolute error persisted under `feature_interaction_group_models`; record total extraction and verification time as `timings.interaction_group_model_seconds`
