...
- show a separate `Interaction pairs` dropdown for explicit feature-pair allowlists; adding a pair selects both features. Pair mode sends `feature_interaction_pairs`, allows grouping constraints only when selected groups are disjoint from every paired feature, allows singleton locks only outside the pair list, persists as `mode: "pairs"`, and converts to LightGBM pair constraints plus any disjoint group constraints plus singleton constraints for selected features that are not covered by a pair or group. Use this mainly for 3-leaf GBM/EBM workflows; with more leaves, warn that pairs constrain branch co-occurrence but do not guarantee purely 2D terms.
