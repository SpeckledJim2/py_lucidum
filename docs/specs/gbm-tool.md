...
- at the top of the interaction-constraint dropdown, an opt-in `Create constraint group LightGBM model .txt file(s)` checkbox requests one SHAP-centred LightGBM text sidecar for every selected group. It defaults off, is enabled only with at least one selected group and non-zero SHAP rows, and disables the `SHAP rows: 0` choice while checked. Trained groups show either the verified maximum absolute raw-score error or `No trees`; edit-time group changes hide results that no longer describe the draft selection.
